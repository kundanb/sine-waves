...
class Control {
  ...
  setVal = (val: number) => {
    this.val = val
    this._element.value = val.toString()
    localStorage.setItem(this._id, this.val.toString())
  }

  reset = () => {
    this.setVal(this.def)
  }
}
...
const smooth = new Control('smooth')
...
const animate = (frame = 0) => {
  ...
  context.save()
  context.fillStyle = `rgba(0, 0, 0, ${1 - smooth.val / 10})`
